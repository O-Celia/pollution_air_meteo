...
# Dossiers
export_folder = "../exports/intermediaire/modelisation"
...
for p in polluants:
    ...
    for model_name, model in models.items():
        ...
        with open(
            os.path.join(export_folder, f"{p}_{model_name}_report.txt"), "w"
        ) as f:
...
df_results = pd.DataFrame(
    results,
    columns=["Polluant", "Modèle", "Accuracy", "Precision", "Recall", "F1-score"],
)
...
# Résumé global
df_results.to_csv(
    os.path.join(export_folder, "resultats_classification.csv"),
    index=False,
    encoding="utf-8-sig",
)
print("\n=== Sauvegarde terminée ===")
